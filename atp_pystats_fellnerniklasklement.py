...
# ════════════════════════════════════════════════════════════════
#  ██  DISTRIBUTIONAL DIAGNOSTICS (cited in §3.4)
# ════════════════════════════════════════════════════════════════
section("DISTRIBUTIONAL DIAGNOSTICS — Ce (All PM bids, CT≥2)")
 
from scipy.stats import shapiro, skew, kurtosis as sp_kurtosis
 
sw_stat, sw_p = shapiro(all_inc)
skewness      = skew(all_inc)
excess_kurt   = sp_kurtosis(all_inc)          # Fisher: excess kurtosis (normal = 0)
 
print(f"""
  Shapiro-Wilk normality test:
    W = {sw_stat:.3f},  p = {sw_p:.3f}  {stars(sw_p)}
    {"→ Normality rejected at α=5% — bootstrap CIs are the primary inference tool." if sw_p < 0.05
      else "→ Normality not rejected at α=5%."}
 
  Skewness (Fisher):          {skewness:+.3f}
  Excess kurtosis (Fisher):   {excess_kurt:+.3f}
 
  Note: CLT applies for N=102. Parametric and bootstrap 95% CIs are
  near-identical (reported under H1), supporting use of the t-test.
""")
...
for label, d in [("Term 1 (CT=2, first re-elec)", t1_ce),
    ...
    print(f"    {label}: mean = {d.mean():+.3f} pp,  t = {tt:.3f},  "
          f"p(2s) = {pp:.5f},  p(1s) = {pp1:.5f}  {stars(pp1)}")
...
for label, d in [
    ...
    print(f"  {label} (N={len(d):>3}): mean={d.mean():+.3f} pp, "
          f"vs 0: p(2s)={p_d:.4f}, p(1s)={p1_d:.4f}{stars(p1_d)}, "
          f"vs bench: p(2s)={p_b:.4f}, p(1s)={p1_b:.4f}{stars(p1_b)}")
...
# ════════════════════════════════════════════════════════════════
#  ██  TERM-LENGTH DECOMPOSITION (§4.6)
# ════════════════════════════════════════════════════════════════
section("TERM-LENGTH DECOMPOSITION (§4.6) — Does the cost grow with tenure?")
print("""
  Theory (Stevenson 2002; N&P median-gap): cost should grow with tenure.
  Test: compare CT=2 (first re-election) vs CT>=3 (subsequent re-elections).
""")
 
for label, d in [("CT=2  (1st re-election)", t1_ce),
                 ("CT>=3 (2nd+ re-election)", t2p_ce)]:
    t_d, p_d   = stats.ttest_1samp(d, 0)
    p1_d       = p_one_less(t_d, p_d)
    t_b, p_b   = stats.ttest_1samp(d, BENCHMARK)
    p1_b       = p_one_less(t_b, p_b)
    se_d       = d.std() / np.sqrt(len(d))
    ci_d       = stats.t.interval(0.95, df=len(d)-1, loc=d.mean(), scale=se_d)
    print(f"  {label}  (N={len(d):>2})")
    print(f"    Mean Ce = {d.mean():+.3f} pp  |  Median = {d.median():+.3f} pp  |  SD = {d.std():.3f} pp")
    print(f"    vs. zero:      t = {t_d:+.3f},  df = {len(d)-1},  p(2s) = {p_d:.5f}  {stars(p_d)}")
    print(f"    vs. benchmark: t = {t_b:+.3f},  df = {len(d)-1},  p(2s) = {p_b:.5f}  {stars(p_b)}")
    print(f"    95% CI: [{ci_d[0]:+.3f}, {ci_d[1]:+.3f}]")
    print()
 
diff_ct   = t2p_ce.mean() - t1_ce.mean()
t_ct, p_ct = stats.ttest_ind(t1_ce, t2p_ce)
print(f"  Difference (CT>=3 mean - CT=2 mean): {diff_ct:+.3f} pp")
print(f"  Two-sample t-test (CT=2 vs CT>=3):   t = {t_ct:.3f},  p(2s) = {p_ct:.5f}  {stars(p_ct)}")
print()
if p_ct >= ALPHA:
    print("  -> NULL FINDING: The difference between CT=2 and CT>=3 is NOT statistically")
    print("     significant in this sampe.")
 
else:
    print(f"  -> SIGNIFICANT: CT>=3 cost is {abs(diff_ct):.2f} pp {'more' if diff_ct < 0 else 'less'} negative than CT=2.")
...
print(f"""
  H1 — PM INCUMBENCY COST IS NEGATIVE:
    ...
    t = {t_h1:.3f},  p(2-sided) = {p_h1_2s:.5f},  p(1-sided) = {p_h1_1s:.5f}  {stars(p_h1_1s)}
  ...
  H2 — PM COST MORE NEGATIVE THAN BENCHMARK ({BENCHMARK:+.2f} pp):
    ...
    → {'CONFIRMED ✓' if p_h2_1s_less < ALPHA else 'NOT CONFIRMED at α=5%'}
